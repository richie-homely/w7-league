    ...
    >
      <div style={{ minWidth: 0 }}>
        <div style={{ marginBottom: 4 }}>
          <TeamName team={team(winnerId)} size="sm" showRatings />
        </div>
        <div style={{ opacity: 0.65 }}>
          <TeamName team={team(loserId)} size="sm" mute showRatings />
        </div>
      </div>
    ...
    <div style={{ padding: "24px 20px", maxWidth: 900, margin: "0 auto" }}>
      ...
      >
        ...
        <div style={{ fontSize: 12, color: C.mute }}>
          {totalPlayed} of {divFixtures.length} matches played
        </div>
      </div>
          ...
          {!query && latest.length > 0 && (
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {latest.map((f) => (
                  <ResultRow key={f.id} fixture={f} divTeams={divTeams} />
                ))}
              </div>
            </div>
          )}
