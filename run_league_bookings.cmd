...
set PYTHONUTF8=1
cd /d "%~dp0"
rem Log the run: under Task Scheduler there is no console, so a failure is otherwise
rem invisible - the task just reports a non-zero result with nothing to read.
python scripts\league_bookings.py --days 21 --push > "%~dp0data\league_bookings.log" 2>&1
echo exit=%ERRORLEVEL% >> "%~dp0data\league_bookings.log"
exit /b %ERRORLEVEL%
