...
def main():
    ...
    for m in matches:
        if m["status"] != "disputed" or (m["box"] >= 90 and not include_test):
            continue
        key = f"disputefollow:{m['id']}:{m['updated_at']}"
        if key in state:
            continue
        mins = (now_utc - datetime.fromisoformat(m["updated_at"].replace("Z", "+00:00"))).total_seconds() / 60
        if mins < DISPUTE_FOLLOWUP_MIN:
            continue
        t1, t2 = teams.get(m["team1_id"]), teams.get(m["team2_id"])
        if not t1 or not t2:
            continue
